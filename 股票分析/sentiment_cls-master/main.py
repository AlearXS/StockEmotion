...
if __name__ == '__main__':
    ...
    for epoch in range(1):
        ...
        print('here')
        torch.save(sentiment_cls.state_dict(), 'sentiment_classifier.pth')

        # 预测逻辑
    sentiment_cls.load_state_dict(torch.load('sentiment_classifier.pth'))
    predict_file_path = "新闻分词同日期待预测情感.txt"
    predict_df = pd.read_csv(predict_file_path, header=None)
    predict_df.columns = ['content']

    sentiment_cls.eval()  # 设置模型为评估模式
    predictions = []

    for index, row in predict_df.iterrows():
        row = row['content']
        tokenized_text = tokenizer(row.strip(), max_length=50, padding='max_length', truncation=True,
                                   return_tensors='pt')
        inputs = {
    ...
    for i, pred in enumerate(predictions):
        print(f"Sample {i + 1}: {'Positive' if pred == 1 else 'Negative'}")