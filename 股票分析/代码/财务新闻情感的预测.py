...
# 读取输入文件
input_file = '../数据/模型输入/测试数据/input_data情感.csv'  # 请确保文件路径正确
...
for i in range(lookback, len(data)):
    ...
    test_samples2 = test_data[:, 12:data.shape[1] - 1].reshape(1, lookback, 83)
...
plt.axhline(y=0.5, color='r', linestyle='--')  # 在y轴0.5处画一条红色虚线
plt.text(0.05, 0.95, f'Accuracy: {accuracy * 100:.2f}%', transform=plt.gca().transAxes, fontsize=12, verticalalignment='top')
plt.show()
