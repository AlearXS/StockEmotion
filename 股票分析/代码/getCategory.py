...
def predict_unseen_data(trained_dir, test_file, date):
    ...
    predicted_dir = '../数据/辅助数据/预测结果/predicted_results/'
...
if __name__ == '__main__':
    # 载入自定义词典
    temp_file = "../数据/辅助数据/temp_file.csv"
    jieba.load_userdict("../数据/辅助数据/stock_dict.txt")
    stop_words = pd.read_csv('../数据/辅助数据/stopwords2.txt', lineterminator="\n", encoding='utf-8', header=None,
                             error_bad_lines=False)
    stopwords = (stop_words[0]).tolist()
    title_file = '../数据/新闻数据分词.csv'
    train_dir = "../数据/辅助数据/训练结果/trained_results_20240630_143942/"
    date = "2024-7-3"
    df = pd.read_csv(title_file, sep=',', encoding="utf-8", engine="python")

    outs = [list(df.columns)]
